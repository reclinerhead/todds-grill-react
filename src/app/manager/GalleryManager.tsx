...
import { uploadGalleryImage, deleteGalleryImage } from "@/app/actions/manager";
...
export default function GalleryManager({
  ...
  async function handleUpload() {
    ...
    if ("error" in result) {
      ...
      setImages((prev) => [{ name: result.name, url: result.url }, ...prev]);
      setUploadFile(null);
  ...
  async function handleDeleteConfirm() {
    ...
    setDeleting(true);
    setDeleteError(null);
    const result = await deleteGalleryImage(deleteTarget.name);
    if ("error" in result) {
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-4 mb-10 max-w-xs">
        <div className="rounded-xl bg-gray-800 border border-white/10 p-5 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">Gallery photos</p>
        </div>
      </div>
